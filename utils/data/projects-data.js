import ayla from '/public/image/ayla.jpg';
import crefin from '/public/image/crefin.jpg';
import realEstate from '/public/image/real-estate.jpg';
import travel from '/public/image/travel.jpg';

export const projectsData = [
    {
        id: 1,
        name: 'VictoryXR',
        description: "Here are links to the programs I worked on while working at VictoryXR. I handled the porting & refactoring of ~40 VR applications & I handled the revitalization & testing of 2 mobile AR applications. For all projects I was incharge of QA testing & debugging, revising & updating project summaries, status reports, & step-by-step guides to key tasks.",
        tools: ['Unity', 'Unity Android', 'VR Headset (DPVR)', 'Azure', 'ARCore', 'ARFoundation', 'Custome VR Packages', 'Custome AR Packages', '????SDK PROGRAM???', '####', 'C#'],
        role: 'Unity Developer, Intern',
        code: 'https://play.google.com/store/apps/dev?id=7568583625005785208&hl=en_US&gl=US',
        demo: 'https://www.victoryxr.com/',
        image: crefin,
    },
    {
        id: 2,
        name: 'VR Paper Airplanes',
        description: 'A VR game where the user must throw paper airplanes into trash bins to score points. #####',
        tools: ['Unity', 'Oculus Quest', "OpenXR", "XR Interaction Toolkit", "C#"],
        role: 'Unity Developer',
        code: '',
        demo: '',
        image: crefin,
    },
    {
        id: 3,
        name: 'VR OSHA Violation Sim.',
        description: 'Project Description.',
        tools: ['Unity', 'Oculus Quest', 'OpenXR', 'XR Interaction Toolkit', 'Unity Store Assets', 'C#'],
        role: 'Unity Developer',
        code: '',
        demo: '',
        image: crefin,
    },
    {
        id: 4,
        name: 'Castle Defender',
        description: "Project Description.",
        tools: ['Unity', 'C#', 'Pixilart', 'MSPaint', "Professors Netcode", 'Unity Store Assets', '###check packages###'],
        role: 'Unity Developer, Pixil Artist, Animator, Programer',
        code: '',
        demo: '',
        image: crefin,
    },
    {
        id: 5,
        name: 'Space Game',
        description: "Project Description.",
        tools: ['Unity', '####Unity HDRP####', 'Blender', 'Unity Store Assets', '####Music/sound Program###', '###Packages###'],
        role: 'Unity Developer, Artist, Sound/Music Designer, Programer',
        code: '',
        demo: '',
        image: crefin,
    },
    {
        id: 6,
        name: 'HTML Maze Game',
        description: "Project Description.",
        tools: ['###Visual Studios###', 'HTML', 'JavaScript', 'MSPaint', '####thing to put pics in####'],
        role: 'MY ROLE',
        code: '',
        demo: '',
        image: crefin,
    },
    {
        id: 7,
        name: 'HTML Paint Program',
        description: "Project Description.",
        tools: ['###Visual Studios###', 'HTML', 'JavaScript'],
        role: 'MY ROLE',
        code: '',
        demo: '',
        image: crefin,
    },
    {
        id: 8,
        name: 'Binary Search Tree Program',
        description: "Project Description.",
        tools: ['Visual Studios', 'C'],
        role: 'MY ROLE',
        code: '',
        demo: '',
        image: crefin,
    },
    {
        id: 9,
        name: 'AVL Tree Program',
        description: "Project Description.",
        tools: ['Visual Studios', 'C'],
        role: 'MY ROLE',
        code: '',
        demo: '',
        image: crefin,
    },
    {
        id: 10,
        name: 'Airline Reservation System',
        description: "Project Description.",
        tools: ['NetBeans','Java', 'JavaFX'],
        role: 'MY ROLE',
        code: '',
        demo: '',
        image: crefin,
    },
    {
        id: 11,
        name: 'OpenCV Project(s)',
        description: "Project Description.",
        tools: ['Visual Studios', 'OPenCV', 'C++'],
        role: 'MY ROLE',
        code: '',
        demo: '',
        image: crefin,
    },
    {
        id: 12,
        name: 'Other Projects',
        description: "If you would like to see my other projects please click one of the links below!",
        tools: [' '],
        role: '',
        code: '',
        demo: '',
        image: crefin,
    }

];


// Do not remove any property.
// Leave it blank instead as shown below

// {
//     id: 1,
//     name: '',
//     description: "",
//     tools: [],
//     role: '',
//     code: '',
//     demo: '',
//     image: crefin,
// },